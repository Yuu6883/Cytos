...
TemplateEngine<T>::TemplateEngine(Server* server, uint16_t id)
    : Engine(server, id),
    ...
    this->tree = new LooseQuadTree<0.25f>(
        IRect(0, 0, dim, dim), T.QUADTREE_MAX_LEVEL, T.QUADTREE_MAX_ITEMS);
    this->dualEnabled = T.DUAL_ENABLED;
    this->defaultCanSpawn = T.PLAYER_CAN_SPAWN;
    this->desiredBots = T.BOTS;

#ifdef WIN32
    pool = static_cast<Cell*>(_aligned_malloc(poolSize(), CELL_T_SIZE));
    boosts = static_cast<Boost*>(_aligned_malloc(boostSize(), 64));
#else
    pool = static_cast<Cell*>(aligned_alloc(CELL_T_SIZE, poolSize()));
    boosts = static_cast<Boost*>(aligned_alloc(64, boostSize()));
#endif
...
void TemplateEngine<T>::restart(bool clearMemory) {
    if (clearMemory) {
        if (pool) memset(pool, 0, poolSize());
        if (boosts) memset(boosts, 0, boostSize());
    }
...
void TemplateEngine<T>::kill(Control* control, bool replace) {
    constexpr float MASS_THRESH =
        (100.f + T.PLAYER_SPAWN_SIZE) * (100.f + T.PLAYER_SPAWN_SIZE) * 0.01f;
    ...
    for (uint32_t _ = 0; _ < server->threadPool->size(); _++) {
        server->threadPool->enqueue([&] {
            ...
            while (true) {
                ...
                bool ejectPerk =
                    c->handle ? (c->handle->perms & EJECT_PERKS) : false;
        ...
        server->threadPool->enqueue([&] {
            while (true) {
                ...
                const bool invincible =
                    c->handle ? c->handle->perms & INVINCIBLE : false;
            ...
            while (true) {
                ...
                for (auto cell : c->sorted) {
                    ...
                    if (f & REMOVE_BIT) {
                        ...
                        const bool noPop =
                            (c->handle && c->handle->perms & NO_POP) ||
                            c->overwrites.cells == 1;
                        auto maxCells = c->overwrites.cells;
                        if (maxCells <= 0) maxCells = T.PLAYER_MAX_CELLS;
