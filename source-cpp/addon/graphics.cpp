...
#define numField(obj, str) \
    (float)fieldTyped(obj, str, Value)->NumberValue(ctx).ToChecked()

#define indexObj(obj, i) (obj)->Get(ctx, (i)).ToLocalChecked()
#define indexInt(obj, i) indexObj(obj, i)->Int32Value(ctx).ToChecked()
#define indexFloat(obj, i) \
    ((float)indexObj(obj, i)->NumberValue(ctx).ToChecked())
...
void render(const FunctionCallbackInfo<Value>& args) {
    ...
    // render loop
    auto jsPlayerDataArray =
        fieldTyped(clientObj, "playerData", Map)->AsArray();
...
#define UVs(prefix) \
    prefix##_UV_X0, prefix##_UV_Y0, prefix##_UV_X1, prefix##_UV_Y1
...
#define viewbox(k) numField(objField(clientObj, "viewbox"), #k)
    ...
    auto expTex =
        indexTyped(objField(clientObj, "expTex"), (lastRAF / 200) % 4, Object);
    ...
    auto cytTex =
        indexTyped(objField(clientObj, "expTex"),
                   std::max(int32_t((lastRAF / 100) % 14) - 10, 0), Object);
    ...
    const bool renderFood =
        field(objField(settings, "renderFood"), "v")->BooleanValue(iso);
    const bool renderSkin =
        field(objField(settings, "renderSkin"), "v")->BooleanValue(iso);
    const bool renderName =
        field(objField(settings, "renderName"), "v")->BooleanValue(iso);
    const auto massMode = field(objField(settings, "renderMass"), "v")
                              ->Int32Value(ctx)
                              .ToChecked();
    const bool longMass = massMode == 2;

    auto textMin =
        std::min(viewport.r - viewport.l, viewport.t - viewport.b) * 0.03f;
    ...
    auto autoTheme =
        field(objField(themes, "autoTheme"), "v")->BooleanValue(iso);
    auto showInactive = field(objField(themes, "showInactiveTabBorder"), "v")
                            ->BooleanValue(iso);
    auto animateFood = field(fieldTyped(themes, "foodAnimation", Object), "v")
                           ->BooleanValue(iso);

    auto cellBorder =
        !field(clientObj, "spectating")->BooleanValue(iso) &&
        field(objField(themes, "showBorder"), "v")->BooleanValue(iso);
        ...
        if ((type & EJECT_BIT) || (type == PELLET_TYPE)) {
            ...
            if (massMode && r > textMin) {
                ...
                if (longMass || mass < 1000) {
                    ...
                    mass_len =
                        snprintf(mass_buffer, 64, "%.2fM", mass / 1000000);
                } else if (mass > 22500) {
                    mass_len = snprintf(mass_buffer, 64, "%iK",
                                        int(roundf(mass / 1000)));
                } else {
                ...
                for (int i = 0; i < mass_len; i++) {
                    ...
                    width += MASS_GAP * MASS_SCALE_X *
                             state->charText[mass_buffer[i]].width;
                }
...
#undef set
    ...
    if (cells.size() != curr_size) {
        stringstream s;
        s << "Cell cache integrity check failed: " << cells.size()
          << " != " << curr_size;
        string out = s.str();
    ...
    for (uint16_t i = 0; i < curr_size; i++) {
        ...
        if (!subop) {
            ...
            iso->ThrowException(Exception::Error(
                String::NewFromUtf8Literal(iso, "Unexpected subop 3")));
        }
    }
    ...
    if (error) {
        iso->ThrowException(Exception::RangeError(
            String::NewFromUtf8Literal(iso, "Reader error")));
        return;
    } else if (!r.eof()) {
        iso->ThrowException(Exception::RangeError(
            String::NewFromUtf8Literal(iso, "Expecting EOF on reader")));
        return;
    }
...
void getCellColor(const FunctionCallbackInfo<Value>& args) {
    ...
    auto color =
        CELL_COLORS[std::max(0, id + state->color_offset) % COLOR_COUNT];
